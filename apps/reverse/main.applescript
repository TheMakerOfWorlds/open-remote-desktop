...
on runHelper(droppedPaths)
	...
	on error errorMessage number errorNumber
		if errorNumber is not -128 then
			do shell script "/usr/bin/logger -t CopyToPrimaryMac " & quoted form of ("helper exited " & errorNumber & ": " & errorMessage)
		end if
	end try
end runHelper
...
on helperPath()
	...
	return appBundlePath & "Contents/MacOS/Copy to Primary Mac"
end helperPath
